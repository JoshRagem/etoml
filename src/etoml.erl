...
-ifdef(TEST).
-define(TESTMSG, "# This is a TOML document. Boom.
...
-define(PARSE2RESULT, [{[<<"title">>],<<"TOML Example">>},
...
file_test() ->
    ...
    meck:expect(file, read_file, fun (_) -> {ok, ?TESTMSG} end),
    FileResult1 = file(<<"./my/file.toml">>),
    FileResult2 = file2(<<"./my/file.toml">>),
    meck:unload(file),
    [?_assert(FileResult1 =:= {ok, ?PARSE1RESULT})
    ,?_assert(FileResult2 =:= {ok, ?PARSE2RESULT})
    ].

parser_test() ->
    [?_assert(parse(?TESTMSG) =:= ?PARSE1RESULT)
    ,?_assert(parse2(?TESTMSG) =:= ?PARSE2RESULT)].

duplicated_test() ->
    [?_assert({error, {duplicated_key, <<"b">>}} =:= parse("b = 1\n b = 1\n"))
    ,?_assert({error, {duplicated_key, <<"a.b">>}} =:= parse("[a]\n b = 1\n b = 1\n"))
    ,?_assert({error, {duplicated_key, <<"a.b">>}} =:= parse("[a.b]\n c = 1\n [a]\n b = 1\n"))
    ,?_assert({error, {duplicated_key, <<"a.b">>}} =:= parse("[a]\n b = 1\n [a.b]\n c = 1\n"))
    ].

array_test() ->
    [?_assert({ok,[{<<"a">>, [<<"a b">>,true,1,[1,2,3,[true,false]],false]}]} =:= parse("a=  [\"a b\" ,true,  1 , [1, 2 , 3,  [true, false]  ,] , false, ] "))].
